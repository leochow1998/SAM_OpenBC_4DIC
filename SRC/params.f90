...
! leo: open_boundary_y_KW78.f90
logical:: doopeny = .false. ! leo: y-bc

! leo: read4dcampinput_new.f90 (in grid.f90)
!logical:: docam4diopdata = .false. ! read the initial condition from 
!                                   ! CAM 4d hist output

! leo: write_fields3D_new.f90 parameters
integer:: micro_out_freq = 1 ! frequency to output the hydrometer mixing ratio
                             ! per nsave3D
logical:: micro_nx_out = .false. ! logical whether to output the 
                                 ! hydrometer concentration

! leo: damping_lateral.f90 parameters
logical:: dodampinglateral = .false. ! logical whether to do lateral damping along y
