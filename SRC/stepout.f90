...
use open_boundary_y, only: write_restart_openbc
implicit none	
...
if(mod(nstep,nstat*(1+nrestart_skip)).eq.0.or.nstep.eq.nstop.or.nelapse.eq.0) then
  ...
  call write_restart_openbc() !leo: save restart file for openybc variables
...
if(mod(nstep,nsave3D).eq.0.and.nstep.ge.nsave3Dstart.and.nstep.le.nsave3Dend ) then
  ! determine if the maximum cloud water exceeds the threshold (no negative cl allowed)
