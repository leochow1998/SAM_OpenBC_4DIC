...
use open_boundary_y, only: ymom_openbl ! leo: ybl
use grid, only: rank
...
! leo: ybl
!!! compute the open yBL
if (doopeny) then
  ! read the dvdt that has been backup (now contains damping and damping lateral)
  ! already be done in the ymom_openbl
  ! compute the open yBL at the y boundaries
  call ymom_openbl()
  ! store the data to the backup
  if (rank.lt.nsubdomains_x) then
    dvdtsouthmost(:,1,:) = dvdt(:,1,:,na) ! contains the open bc + damping terms
