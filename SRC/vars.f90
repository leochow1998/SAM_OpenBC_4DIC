...
! to backup the northmost v in Arakawa C grid
real vnorthmost  (dimx1_v:dimx2_v, 1, nzm) ! leo: ybl
! to backup the whole v in Arakawa C grid at the current timestep
! for scalor advection
real ubackup (dimx1_u:dimx2_u, dimy1_u:dimy2_u, nzm)
real vbackup (dimx1_v:dimx2_v, dimy1_v:dimy2_v, nzm)
real wbackup (dimx1_w:dimx2_w, dimy1_w:dimy2_w, nz)
...
real buoy    (nx,ny,nzm)                  ! temperarory array for buoyancy ! leo
...
! to backup the dvdt at the ybl after each update in Arakawa C grid
! which keep the source term / advection term that you want to retain
real dvdtnorthmost (nx,1,nzm) ! leo: ybl
real dvdtsouthmost (nx,1,nzm)
...
real shf_xy(nx,ny) ! mean sensible heat flux for outout
real lhf_xy(nx,ny) ! mean latent heat flux for outout
...
!----------------------------------------------------------------------
! newly added arrays for more output ! leo

! 3D vars
real mheat    (nx,ny,nzm)       ! change of MSE due to microphysics (K/s)
real mtt      (nx,ny,nzm)       ! change of absolute temperature due to microphysics (K/s)
real mtqv     (nx,ny,nzm)       ! change of water vapor mixing ratio due to microphysics (kg/kg/s)
real mtqt     (nx,ny,nzm)       ! change of total nonprecipitating water mixing ratio due to microphysics (kg/kg/s)
real mtqp     (nx,ny,nzm)       ! change of total precipitating water mixing ratio due to microphysics (kg/kg/s)
! 2D vars
real taux_xy  (nx,ny)           ! zonal surface stress (N/m2)
real tauy_xy  (nx,ny)           ! meriodional surface stress (N/m2)
