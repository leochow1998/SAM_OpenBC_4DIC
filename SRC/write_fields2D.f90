...
nfields= 34 ! original: 30
            !  (added: taux, tauy, latitude, longitude)
...
if(SFC_FLX_FXD) nfields = nfields-4 ! from 2 to 4 (! leo), also write the taux and tauy
nfields1=0
...
! only for averaging along sampling period
if(save2Davg) then
...
if(.not.SFC_FLX_FXD) then
  ...
  do j=1,ny
    do i=1,nx
      tmp(i,j,1) = taux_xy(i,j)*rhow(1)*coef
      taux_xy(i,j) = 0.
    end do
  end do
  name='TAUX'
  ...
  do j=1,ny
    do i=1,nx
      tmp(i,j,1) = tauy_xy(i,j)*rhow(1)*coef
      tauy_xy(i,j) = 0.
    end do
  end do
  name='TAUY'
  ...
  units='N/m2'
  call compress3D(tmp,nx,ny,1,name,long_name,units, &
                               save2Dbin,dompi,rank,nsubdomains) ! leo --
