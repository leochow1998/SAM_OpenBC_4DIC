...
! (leo Chow)
! if docam4diopdata = .true. replace all the 3D arrays from
! a netcdf dataset with all the required variables
! pre-interpolated to the 3D grid domain of SAM
if(docam4diopdata) then 
  call read4dcaminput()
  ! reinitialize the microphysics diagnostic profile 
  ! (already done in read4dcaminput), qc0 and qi0 is internal
!  do k=1,nzm
!    qc0(k) = sum(dble(qcl(1:nx,1:ny,k)))/float(nx*ny)
!    qi0(k) = sum(dble(qci(1:nx,1:ny,k)))/float(nx*ny)
!    qn0(k) = qc0(k) + qi0(k)
!    qv0(k) = q0(k) - qn0(k)
!    qp0(k) = sum(dble(qpl(1:nx,1:ny,k)))/float(nx*ny) + &
!             sum(dble(qpi(1:nx,1:ny,k)))/float(nx*ny)
!    tabs0(k)= sum(dble(tabs(1:nx,1:ny,k)))/float(nx*ny)
!  end do
end if
...
if(masterproc) then
 ...
 print *, ' k      z    zi     rho     rhoi    s      h     h*      qt      u      v     adz'
 do k=nzm,1,-1
   write(6,'(i4,1x,2f7.1,2f7.3,f7.2,f7.2,4f7.2,4g11.4)') k,z(k),zi(k),rho(k),rhow(k),tabs0(k)+gamaz(k), &
...
if (masterproc) then
  write(*,*) '          k          z           zi             rho             rhoi'
  do k=nzm,1,-1
    write(*,*) k, z(k), zi(k), rho(k), rhow(k)
  end do
end if
