...
real tke(dimx1_s:dimx2_s, dimy1_s:dimy2_s, nzm)   ! SGS TKE leo: (\bar{e'})
equivalence (tke(dimx1_s,dimy1_s,1),sgs_field(dimx1_s,dimy1_s,1,1))

! make aliases for diagnostic variables:

real tk  (dimx1_d:dimx2_d, dimy1_d:dimy2_d, nzm) ! SGS eddy viscosity leo: (Km)
real tkh (dimx1_d:dimx2_d, dimy1_d:dimy2_d, nzm) ! SGS eddy conductivity leo: (Kh)
...
subroutine sgs_scalars()
  ...
  implicit none
    ...
    real :: t0temp(nzm)
      ...
      !------------------------------
      ! 20231013
      ! P.S. don't know why bugs for t0, but will change after 
      ! calling diffuse_scalar3D first dowally
      ! so backup the t0 here and retain it after the call
      t0temp(:) = t0(:)
