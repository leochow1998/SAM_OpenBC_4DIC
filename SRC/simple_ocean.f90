...
SUBROUTINE set_sst()
 ...
 use vars, only: sstxy,t00, latitude
 use params, only: tabs_s, delta_sst, ocean_type, latitude0 ! (Leo Chow)

! parameters of the sinusoidal SST destribution 
! along the X for Walker-type simulatons( ocean-type = 1):

 real(8) tmpx(nx), pii, lx, yy
 real latit, tmp, sst_tmp ! (Leo Chow)
 integer i,j, it,jt
 ...
 select case (ocean_type)
  ...
  case(4) ! Qobs profile in aquaplanet protocol (Neale and Hoskins 2000)
          ! with maximum T at 10N, maximum T = 29degC
   
    call task_rank_to_index(rank,it,jt)

    pii = atan2(0.d0,-1.d0) ! pi
    do j=1,ny
      !latit=latitude0+dy*(j+jt-(ny_gl+YES3D-1)/2-1)*2.5e-8*360. ! degree (2.5e-8 = 1/(6371000*2*pi))
      latit = latitude(1,j) ! zonally symmetric, degree
      tmp = sin(3./2.*(latit-10.)*pii/180.)
      sst_tmp = 0. + (29. - 0.) * ((1.-1./2.*tmp*tmp) - &
                                   (1./2.*tmp*tmp*tmp*tmp)) + 273.15 - t00 ! 300 K: constant offset
      do i=1,nx
        sstxy(i,j) = sst_tmp
      end do
    end do
