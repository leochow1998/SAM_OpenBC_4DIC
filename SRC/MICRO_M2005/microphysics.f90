...
!bloss/qt  if(docloud) lfac(iqcl) = lcond
 ...
 ! compute initial profiles of liquid water - M.K.
      call satadj_liquid(nzm,tabs0,q0,qc0,pres*100.,0,0)
...
use vars, only: t,  gamaz, precsfc, precflux, qpfall, tlat, prec_xy, &
     ...
     mheat, mtt, mtqv, mtqt, mtqp ! leo (temperature and microphysics tendency output)
...
!      tmpw = 0.5*(w(i,j,1:nzm) + w(i,j,2:nz))  ! MK: changed for stretched grids 
      ...
      call satadj_liquid(nzm,tmptabs,tmpqv,tmpqcl,tmppres,i,j)
     ...
     ! update microphysical quantities in this grid column
      ...
      !=====================================================
      ! leo
      ! store the change of MSE due to microphy processes
      ! for output in 3D
      ! P.S. the rho is in stendq**
      mheat(i,j,:) = -1. * fac_cond*(stendqcl+stendqr) &
                     - fac_sub*(stendqci+stendqs+stendqg)
      ! temperature tendency due to microphysics
      ! (sensible heating due to phase change)
      mtt(i,j,1:nzm) = tmtend1d(1:nzm)
      ! water vapor mixing ratio due to microphysics
      mtqv(i,j,1:nzm) = mtendqv(1:nzm)
      ! total nonprecipitating water due to microphysics
      ! (cloud liquid + cloud ice)
      mtqt(i,j,1:nzm) = (mtendqcl(1:nzm) + mtendqci(1:nzm))
      ! total precipitating water due to microphysics
      ! (rain + snow + graupel)
      mtqp(i,j,1:nzm) = (mtendqs(1:nzm) + mtendqr(1:nzm) + mtendqg(1:nzm))
      !=====================================================
...
!-----------------------------------------
subroutine satadj_liquid(nzm,tabs,qt,qc,pres,iin, jin) ! leo
  ...
  use grid, only: rank, nc !Leo
  use vars, only: dwdt, v ! leo
  ...
  do k = 1,nzm
    ...
    if (qt(k).gt.qsat1) then
      ...
      if (niter.gt.maxiter-1) then ! leo
         call task_rank_to_index(rank,itt,jtt)
         write(*,*) 'Reached iteration limit in satadj_liquid', rank, itt+iin, jtt+jin, &
                                                                k, qc(k), qt(k), tabs(k), tabs1, dwdt(iin, jin, k, nc), &
                                                                v(iin,jin,k), v(iin,jin-1,k)
      end if 
