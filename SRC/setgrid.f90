...
! earth average radius (degree)
integer :: earth_avg_radius = 6371008.7714
...
if(dofplane) then
 ...
 do j=0,ny
     ...
     ! change to mercator projection to compute the latitude ! leo
     ! for the index, minus one more 1 to correct to python 0 to 2160 exclusive index
     latit = (2.*atan(exp((dy*(j+jt-(ny_gl+YES3D-1-1)/2-1) &
                           +(earth_avg_radius*log(tan(pi/4.+latitude0/180.*pi/2.)))) &
                          /earth_avg_radius)) - pi/2.) & 
              / pi * 180. ! degree
...
if (doradlat) then
 ...
 do j=1,ny
     ...
     ! use the mercator projection to compute the latitude ! leo
     ! for the index, minus one more 1 to correct to python 0 to 2160 exclusive index
     latitude(:,j) = (2.*atan(exp((dy*(j+jt-(ny_gl+YES3D-1-1)/2-1) &
                                   +(earth_avg_radius*log(tan(pi/4.+latitude0/180.*pi/2.)))) &
                                  /earth_avg_radius)) - pi/2.) & 
                      / pi * 180. ! degree
  end do
...
if (doradlon) then
 ...
 do i=1,nx
     ...
     ! use the mercator projection to compute the longitude ! leo
     ! by test, the index is correct here
     longitude(i,:) = longitude0 + & 
                      ((dx*(i+it-nx_gl/2-1) / earth_avg_radius) / pi * 180.)
  end do
else
  longitude(:,:) = longitude0
end if
